...
export default function CourseFileLibrary(){
  ...
  console.log('useruser123:', userId)
  ...
  const handleGetCourseFiles = async(name) => {
    ...
    let response = await new FilesAPI().getAddminFiles(userId, data)
    // setLoading(false)
    if(response.ok){
      setDisplayFiles(response?.data?.files)
      setDisplayFolder(response?.data?.folders)
    }else{
      alert("Something went wrong while fetching Course files.1111")
    }
  }
  ...
  const handleClickedBreadcrumbsItem = (value, index) => {
	...
	return (
    <div>
      <FileHeader type={'AdminFiles'} title='Files' id={userId} subFolder={subFolderDirectory.join('')} doneUpload={()=> handleGetCourseFiles()}/>
      <div>
